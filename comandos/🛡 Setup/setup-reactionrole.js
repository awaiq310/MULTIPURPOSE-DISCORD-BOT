...
module.exports = {
    ...
    desc: "Used to ver crear un sistema de roles con reacciones",
    ...
    run: async (client, message, args, prefix) => {
        ...
        async function finalizar() {
            if(contador === 1 && !objeto.Parametros.length) return message.reply(`❌ **You have to añadir al menos un emoji con un rol**\nSetup cancelado!`);
                ...
                if(canal) {
                    ...
                    await idmensaje.channel.awaitMessages({
                        ...
                        if(found){
                            ...
                            return message.reply(`❌ **No se ha found el mensaje que has specified!**\nSetup cancelado!`)
                        }
                    }).catch((e) => {
                        return message.reply(`Tu tiempo ha expirado!`)
                    })
                } else {
                    return message.reply(`❌ **No se ha found el canal que has specified!**\nSetup cancelado!`)
                }
            }).catch((e) => {
                return message.reply(`Tu tiempo ha expirado!`)
            })
