...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        //definimos los mealls del sorteos
        let mealls = ["start", "reroll", "end"];
        if(!args || !mealls.includes(args[0])) return message.reply({
            embeds: [new Discord.EmbedBuilder()
            .setTitle(`❌ You have to especificar un méall válido!`)
            .setColor("FF0000")
            .setDescription(`Méalls disponibles: ${mealls.map(meall => `\`${meall}\``).join(", ")}`)
            ]
        });
        ...
        switch (args[0]) {
            case "start":{
                ...
                if(!canal) return message.reply({
                    embeds: [embed.setTitle(`❌ You have to especificar un canal válido!`)]
                })
                let tiempo = args[2];
                if(!tiempo) return message.reply({
                    embeds: [embed.setTitle(`❌ You have to especificar una duración del sorteo válida!`)]
                })
                ...
                if(!ganadores || isNaN(ganadores) || ganadores < 0 || ganadores % 1 != 0) return message.reply({
                    embeds: [embed.setTitle(`❌ You have to especificar una amount de ganadores válida!`)]
                })
                let premio = args.slice(4).join(" ");
                if(!premio) return message.reply({
                    embeds: [embed.setTitle(`❌ You have to especificar un premio válido!`)]
                });
